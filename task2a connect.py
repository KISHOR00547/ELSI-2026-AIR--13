# connector.py — communication layer for bridge_v1_2a.py (PID + Pick & Place).
#
# Talks to bridge_v1_2a.py over TCP (127.0.0.1:50002) using its text protocol.
...
class CoppeliaClient:
    """
    Text-protocol client for bridge_v1_2a.py.
    ...
    def receive_sensor_data(self):
        """Return the newest parsed sensor dict, or None if no packet yet.

        The bridge streams sensor packets faster than a typical control loop
        reads them. This drains every complete line currently buffered and
        returns the MOST RECENT one (discarding older ones), so the control
        loop always acts on fresh data instead of a backlog that grows — and
        lags — without bound.
        ...
        while "\n" in self.buffer:
            ...
            if parsed is not None:
                self._recv_count += 1
                latest = parsed   # keep the freshest; don't return early
        return latest
    ...
    def _wait_for_reply(self, prefix, timeout=2.0):
        ...
        Non-reply lines (sensor packets) that arrive while waiting are set aside
        and re-queued into self.buffer afterwards, so receive_sensor_data() can
        still see them. They must NOT be pushed back onto self.buffer mid-scan:
        doing so would re-split the same line forever (an infinite loop that
        never rechecks the timeout).
        """
        deadline = time.time() + timeout
        leftover = []   # sensor / other lines to re-queue once we're done
        while time.time() < deadline:
            ...
            while "\n" in self.buffer:
                ...
                if line.startswith(f"{prefix}:"):
                    ...
                    if leftover:   # restore set-aside sensor lines for later reads
                        self.buffer = "\n".join(leftover) + "\n" + self.buffer
                    return val == "true"
                # set aside (do NOT re-buffer in place -> avoids infinite loop)
